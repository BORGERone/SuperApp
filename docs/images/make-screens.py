...
def draw_drive():
    ...
    # кнопки панели — прижаты вправо, как в макете и клиенте
    rx = W - 25
    ...
    tile_size = 148
    avail_w = (W - 25) - x0
    gap_x = (avail_w - tile_cols * tile_size) / (tile_cols - 1)
    gap_y = 24
    start_x = x0
    start_y = 196
    ...
    for r in range(tile_rows):
        for c in range(tile_cols):
            tx = start_x + c * (tile_size + gap_x)
            ty = start_y + r * (tile_size + gap_y)
            # Квадратная белая карточка с мягкой тенью
            card(img, [tx, ty, tx + tile_size, ty + tile_size], r=20, shadow=(14, 6, 20))
            cx = tx + tile_size / 2
            cy = ty + tile_size / 2
            if r < 2:
