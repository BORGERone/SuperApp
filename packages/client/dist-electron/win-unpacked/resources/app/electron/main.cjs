const { app, BrowserWindow, ipcMain, dialog, Tray, Menu, nativeImage, safeStorage, protocol } = require('electron');
...
// Регистрируем кастомный протокол для загрузки файлов из app.asar
app.whenReady().then(() => {
  protocol.registerFileProtocol('superapp', (request, callback) => {
    const filePath = request.url.substring(11); // Убираем 'superapp://'
...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      webSecurity: false, // Отключаем webSecurity для локальных файлов
    },
  });
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    // В продакшене загружаем собранные файлы через loadFile
    // asar отключен, поэтому файлы находятся в обычной структуре
    ...
    // Временно открываем DevTools для диагностики
    mainWindow.webContents.openDevTools();
  }
